...
def plot_IDS_results(T, n_expe, results, labels, colors, title, log=False):
    """
    ...
    #g = np.empty((T, n_expe))
    IR = np.empty((T*len(labels), n_expe))
    ...
    for i, l in enumerate(labels):
        c = cmap[i] if not colors else colors[i]
        target = np.clip((IR[i*T:(i+1)*T]).mean(axis=1), -5, 5) * 10
        target[(target == 50) | (target == -50)] = 0
        ax1.plot(x, target, c=c, label=l)
        if log:
            plt.yscale('log')
    #ax1.fill_between(x, np.quantile(IR, 0.1, axis=1), np.quantile(IR, 0.9, axis=1), color='#FFA07A')
    ax1.set_title(title) 
    ax1.legend()
    plt.savefig('plots/IDS_results4.png')
