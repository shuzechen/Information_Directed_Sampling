...
class BetaBernoulliMAB(GenericMAB):
    ...
    def TS(self, T):
        ...
        if self.store_IDS:
            ...
            X, f, F, G, B, maap, p_star, prod_F1, g = self.init_approx(10000, beta1, beta2)
        ...
        for t in range(T):
            ...
            if self.store_IDS:
                delta, g, p_star, maap = self.IR_approx(10000, beta1, beta2, X, f, F, G, g)
                ...
                self.IDS_results['IR'].append((np.inner(delta, policy))**2 /np.inner(g, policy))
                # Posterior update
